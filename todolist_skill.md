...
> **文档分工**：最终实现与大纲、快速上手见 [README.md](README.md)；本文档为开发清单与进度、**待实现/待完成**、**遇到的问题与解决方案**、扩展指南。**重要：清单与选型相关更新须记录时间。**  
> **时间约定**：所有时间记录均为**东八区（上海）时间**。  
> **最后更新**：2026-03-16（东八区/北京时间）
...
> **隐私规则**：`prompts.md` 中必须对 API Key、密码、手机号等敏感信息脱敏（如 `sk-****12f9`）。实际密钥只存放在 `.env`，绝不出现在代码或文档。每条记录必须标注日期。

> **提交规则**：用户说「提交」即执行 `git add . && git commit && git push`。提交信息使用中文，遵循 conventional commits 风格。

> **时间记录规则**：每次更新「开发进度」「待完善」「后续模块说明与选型」或「遇到的问题与解决方案」后，须同步更新本文档顶部的 **最后更新** 日期。
...
（遇到新问题并解决后，请在本节追加一条，并更新文首「最后更新」时间。）

| 时间 | 问题 | 解决方案 |
|------|------|----------|
| 2026-03-15 | 外部终端运行 `python run.py --schedule` 报 `ModuleNotFoundError: No module named 'apscheduler'` | 该终端使用的 Python 环境未安装依赖。在该终端执行 `pip install -r requirements.txt` 或先激活项目 venv 再运行。 |
| 2026-03-15 | `scheduler.start()` 报 `RuntimeError: no running event loop` | `AsyncIOScheduler` 需在已运行的事件循环中 start。改为在 `asyncio.run(_run_schedule())` 内先启动循环再 `scheduler.start()`，见 run.py。 |
| 2026-03-15 | Windows 下控制台日志中文乱码（如 ▒▒▒▒▒▒ʱ▒▒▒▒ģʽ） | 在 _setup_logging 中：Windows TTY 下调用 `SetConsoleOutputCP(65001)`，并对 stdout 做 `reconfigure(encoding="utf-8")`，见 run.py。 |
| 2026-03-15 | 项目根目录出现 xDialogRsp.con、xQueryRsp.con、xTradingDay.con | OpenCTP 库在运行 CTP 时于当前工作目录写入。可随时删除；已加入 .gitignore。若不想在根目录生成，可从子目录运行或定期清理。 |
...
- **能跑通整个流程吗？** 能。配置好 `.env` 和 `config/settings.yaml` 后，`python run.py` 走完 8 步；非交易日/非交易时段会跑完 Step 1～7，仅 Step 8 不执行交易。`python run.py --test` 用内置石油新闻跑通全流程。
- **回测是干嘛的？** 用已有交易记录 + 历史日线模拟收益与最大回撤，按需执行 `python run.py --backtest`，不需每天跑定时器。
- **卖出会通知吗？** 会。止盈/止损触发时与买入一样发邮件（及企微/PushPlus），主题与正文会标明「卖出信号(止盈/止损)」。
...
| 模拟盘 API 闭环 | 接口已实现 | `trading.mode: paper` 当前落 JSON+DB；后续在 PaperExecutor 内接入券商模拟盘 API。 |
| Web Dashboard | P3 | 可视化看板。 |

模拟交易与对接（模拟盘/CTP/同花顺等）的详细说明、配置与排查见项目历史版本或另行在本文档中按需补充章节。
