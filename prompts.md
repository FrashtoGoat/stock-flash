...
**原始提示词**（摘要）:
> 自研池有两只股票为什么只有沃尔有推荐；量比换手都是N/A是什么；均线/MACD满足到底是满足什么指标；说好的买入价格建议呢。

**执行结果**:
- **自研池未通过标的**：自研池通知中增加「自研池未通过标的」表格/段落，列出未通过链式筛选的标的及**未通过环节**（如：沪电股份 未通过 技术），便于核对为何只推荐一只。
- **量比/换手 N/A**：技术面在**非交易时段或行情接口未返回**量比/换手时，筛选器将二者视为通过（不因缺数据卡掉标的）；通过标准文案中改为「量比(行情未取到,视为通过)」「换手(行情未取到,视为通过)」，避免误读为异常。
- **均线/MACD 满足**：通过标准中技术面改为写出**具体条件**：MA5/MA10/MA20 数值与是否多头、是否站上 MA20、MACD 与信号线数值及是否满足配置（水上/金叉），不再笼统写「均线/MACD 满足」。
- **建议买卖价**：现价/建议买入价/建议卖出价取不到时（实时行情失败或非交易时段），邮件与短文本中**显式标注「未取到行情」**，避免空白或「—」让人误以为未实现。
...
**原始提示词**（摘要）:
> simulated/paper 希望可选“忽略交易时段也落库/模拟盘”；新闻里不要再推那种“已经大涨/涨停”变成既定事实的内容；新闻利空/宏观分析使用身份 agent `agent/]micro_professor.md`。

**执行结果**:
- **模拟交易强制开关**：新增配置 `trading.force_simulated_trade_outside_hours`。当其为 true 且 `trading.mode` 为 simulated/paper 时，即使不在交易时段也会执行（落库/模拟盘）；ctp/live 仍受交易时段限制。
- **新闻预处理反追涨**：在 `src/news/preprocessor.py` 增加“走势型快讯”过滤（含涨停/大涨/新高等、且缺少订单/业绩/政策等催化词时，判为已涨成事实，直接不送 LLM）。
- **宏观身份接入**：`src/analyzer/bearish_analyzer.py` 在利空分析 system prompt 前自动注入 `agent/]micro_professor.md` 人设，使宏观/政策/事件解读更符合“老宏”风格（仍按原 JSON 输出规范）。
...
**原始提示词**（摘要）:
> 同一种类型放一行好看点；10年期与短期美债收益率都展示；表格和折线图展示在一起。宏观定时改为1天执行1次。提交代码。

**执行结果**:
- **宏观报告**：同一指标内表格与折线图左右并排（月度一行、年份一行）；增加美国2年期/5年期/10年期国债收益率，美联储利率保留；每类指标一块，表与图同屏。
- **宏观定时**：`config/settings.yaml` 宏观任务改为单任务 `macro_daily`，cron `0 8 * * *`，每天 8:00 执行 1 次。
...
**原始提示词**（摘要）:
> 92% 共6环全部通过… 这个环节通过为什么通过，比如基本面你判断的标准是什么，技术又是满足了什么条件，你不解释不就变成黑盒了吗；宏观报告：月份展示最近12个月，年份展示最多过去30年，要求折线图。

**执行结果**:
- **链式筛选可解释**：新增 `src/filters/format_reasons.py`，按 `passed_filters` 与 `details` 生成每环通过依据（板块/可买/异动/基本面/机构/技术）；自研池信号写入 `TradeSignal.filter_pass_reason`，通知中追加「通过标准：…」。
- **宏观报告**：月度改为**最近 12 个月**（不再仅当年），年份保持**近 30 年**；HTML 中为每个指标的月度和年份数据各增加**内联 SVG 折线图**（`_svg_line_chart`），邮件内直接展示曲线。
...
**原始提示词**（摘要）:
> 其实我是想要按照年份和月份的记录，月份就展示当年的，年份就展示最近30年以内如果有的话

**执行结果**:
- **宏观报告改版**：邮件报告不再按「近 N 天」列表，改为按**月度（当年）**、**年份（近30年）**两张表展示。
- **月度**：仅展示当前自然年的数据，按 2026-01～2026-12 等月份聚合，每月的数值取该月内最后一次出现的值（美联储决议/美债日频月末值）。
- **年份**：展示最近 30 年内的数据（若有），按年倒序；每年的数值取该年内最后一次出现的值。
- **实现**：`_usa_fed_rate_by_month_and_year()`、`_usa_bond_yield_by_month_and_year()` 拉取原始序列后经 `_group_by_month_and_year()` 聚合；`fetch_macro_report()` 返回 `{指标: {by_month, by_year}}`；`build_macro_report_html()` 为每个指标生成「月度（当年）」「年份（近30年，如有）」两表。配置去掉 `days_for_report`，保留 `notify_email`。
...
**原始提示词**（摘要）:
> 宏观拉取这里能获取一段时间的数据展示成表格并输出到邮箱吗

**执行结果**:
- **一段时间数据**：新增 `_usa_fed_rate_series(days)`、`_usa_bond_yield_series(days)`，拉取最近 N 天美联储利率决议与美债 10Y 日频序列。
- **表格报告**：`fetch_macro_report(days)` 汇总上述序列；`build_macro_report_html(report, days)` 生成 HTML 表格（每指标一表：日期 | 数值）。
- **发邮件**：notifier 新增 `send_custom_email(subject, html_body)`，复用 notification.email 配置。宏观拉取完成后若 `macro.notify_email: true`，则拉取 `macro.days_for_report`（默认 7）天数据、生成表格并发送邮件。
- **配置**：`macro.days_for_report: 7`、`macro.notify_email: true`。
...
**原始提示词**（摘要）:
> 1 实现宏观模块（todolist_skill 中宏观信息模块）  
> 2 为什么我在 README 中有说每次要记录 prompts.md，你为什么没这么做？告诉我原因

**执行结果**:
- **宏观模块已实现**：新建 `src/macro/`，`fetch_macro_snapshot()` 拉取美联储利率（akshare）、美国 10 年期国债收益率（bond_zh_us_rate）；结果落 `result/macro/YYYY-MM-DD/HH.json`。scheduler 新增 3 个 job：每天 8:00、15:00、21:00 各执行一次。配置 `macro.enabled`、`scheduler.jobs` 中 `macro: true`；命令行 `python run.py --macro`。
- **未记录 prompts.md 的原因**：README/todolist 规定「须写入 prompts.md」，但执行依赖**人工/AI 主动追加**，没有自动钩子。此前未每次追加是因为：未把「完成用户需求后」明确为触发动作，导致有时只回复未写文件。已强化：README 与 todolist 中改为「**每次**有用户需求时须写入」「**AI 在完成用户需求后须主动追加**当次需求与执行结果」；本则已补录至 prompts.md。
...
**原始提示词**（摘要）:
> 1 README 再简化，只记录最终实现和大纲，待实现/待完成、遇到的问题与解决方案都记到 todolist_skill，强化记录规则，有提示词要记到 prompts.md  
> 2 对输出结果不满意：要在结果里说明为什么是 92%、原因里满足条件的原因；确保满足卖出条件时也会通知我

**执行结果**:
- **README 简化**：README 仅保留文档分工、8 步流水线、两条路线、已实现概要、快速开始；待实现/常见问题/模拟交易与对接等迁至 todolist_skill.md。
- **todolist_skill**：新增「遇到的问题与解决方案」章节（apscheduler 依赖、事件循环、Windows 乱码、.con 文件）；强化「提示词与执行结果须写入 prompts.md」；新增常见问题、待实现与待完成概要。
- **通知内容**：邮件与企微/PushPlus 增加「信心说明与满足条件」：自研池说明为何是该百分比（如 92%=基础 50%+6 环×7%）及通过环节（板块、可买、异动等）；新闻驱动说明来自 LLM 评分。卖出信号（止盈/止损）单独区块与主题标明「卖出信号(止盈/止损)」，与买入一样发通知。
...
**原始提示词**（摘要）:
> 1 定时器启用时两种路线都跑、互不干扰，8 点到晚上 1 点每隔 1 小时执行；达成交易的数据做回测，初始资金分两份每条路线一份，最低买入金额算上手续费滑点不亏，按评分决定金额不能 all in  
> 2 提示词工程怎么不写入了；日志要打印当前获取到的股票价格及是否成功

**执行结果**:
- **定时双路线**：scheduler 支持 `run_both: true`，cron `0 0,1,8-23 * * *`，先 pipeline 再 pipeline_research_pool。
- **交易与回测**：配置 min_buy_amount/max_amount_per_trade/score_based_amount、initial_capital 与两条路线资金比例；信号金额按评分计算；回测按 source 分两条资金曲线。
- **提示词与 README**：README 开发规则中写明提示词须记录到 prompts.md；todolist 强化记录规则。
- **行情日志**：get_realtime_quote/get_kline 成功/失败时打日志（代码、最新价或日期、是否成功）。
